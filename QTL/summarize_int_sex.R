...
# The script uses output from scanone_int_sex.R
# For every phenotype the data is stored phenotype__intSex_hglmResPerm.RData
...
final.table.int.sex <- as.data.frame(matrix(nrow=1,ncol=12))
colnames(final.table.int.sex) <- c("PHEN","CHR","SNP","Pos","CI","LOD","Type","VarSex","VarDiet","VarGeno","VarGenoxDiet","VarGenoxSex")
for(k in 5:ncol(ncmball)){
	phen=colnames(ncmball)[k]
	filename.res <- paste("Output/",phen,"_intSex_hglmResPerm.RData",sep="");
	load(filename.res)
...
# Calculate genome-wide significance and suggestive-significance
	...
	final.table.int.sex <- rbind(final.table.int.sex,out.data.var)
	cat(paste(phen,"has been summarized","\n",sep=" "));

}
write.table(final.table.int.sex,file="Summarize_results_int_sex.tab")
